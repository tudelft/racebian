...
# get and apply RT patch. (see kernel Makefile for version)
...
# since 6.12, the RT patch is now merged into mainline, so we just need to enable it in the config
...
make -j16 ARCH=arm64 CROSS_COMPILE=aarch64-linux-gnu- Image modules dtbs bindeb-pkg
